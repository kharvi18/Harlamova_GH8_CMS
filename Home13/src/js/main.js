$(function () {
  ...
  let selectedDay = callendar.datepicker('getDate').toLocaleDateString().replace(/\./g, '-')
  ...
  function adjustTaskInputHeight () {
    $(this).css('height', '1px')
    let height = (5 + $(this).prop('scrollHeight')) + 'px'
    $(this).css('height', height)
  }

  function isInputValid (taskInput) {
    let trimInput = $.trim(taskInput).length
  ...
  function showErrorModal () {
    let modalContent = '<p>The text of the task should contain from 3 to 100 characters</p><button id="ok-btn" class="button button--modal">OK</button>'
  ...
  function setPriority () {
    let select = $('ul select')

    $.each(select, function (i, option) {
      let priorityValue = $(option).prev().prev().data('priority')
      if (priorityValue === 1) {
  ...
  function getTaskIndex (task) {
    let id = task.parent().find('.task').data('id')
  ...
  function saveEditedTask (button) {
    let taskToEdit = singleDayTask[selectedDay][getTaskIndex(button)]
    let taskEditedText = button.prev().text()
  ...
  function removeCompletedTasks () {
    let completeArray = singleDayTask[selectedDay].map(function (singleTask) {
      return singleTask.completed
    })
  ...
  function showAskModal () {
    let modalContent = '<p>Are you sure you want to delete ALL tasks of the day?</p><button id="yes-delete-btn" class="button button--modal">YES</button><button id="no-delete-btn" class="button button--modal">NO</button>'
